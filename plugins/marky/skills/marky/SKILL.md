...
watch doorbell for that canvas (`Monitor` on `marky comments watch <slug> --json`).
"Park it" sets the mode back to `feedback` **and** stops that watch.

**Mode is the source of truth; the watch is just plumbing:**
- Only arm a watch for `live` canvases. A no-slug `marky comments inbox` sweep already
...
**Orient first with `marky context`.** Before a push/update, run `marky context` (or
...
tidy-up only fills **project** and **tags**. (And note: `marky doctor` is a *setup* health check —
...
1. Start `marky comments watch <slug>` in the background.
...
- `marky comments ls <slug> --open --json`, work through each thread, edit the source,
...
- **`marky comments watch <slug> --json`** — the **event** stream. Holds one live
  InstantDB subscription and prints one JSON line per new comment (realtime,
  sub-second, stays alive). This is the *doorbell*.
- **`marky comments inbox [slug] [--json]`** — the **state**. One-shot; returns the full,
